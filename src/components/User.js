...
import { UserType } from "../UserContext";
import userData from './dummyUsers.json'
  ...
  return (
    <Pressable style={{ flexDirection: "row", alignItems: "center", marginVertical: 10 }}>
      ...
      {userFriends.includes(item.id) ? (
        <Button mode="contained" color="#82CD47" style={{ borderRadius: 6 }} disabled>
          Friends
        </Button>
      ) : requestSent || friendRequests.some((friend) => friend.id === item.id) ? (
        <Button mode="contained" color="gray" style={{ borderRadius: 6 }} disabled>
          Request Sent
        </Button>
      ) : (
        <Button
